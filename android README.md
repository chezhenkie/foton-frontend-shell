...
| Language | Where | Size |
| --- | --- | --- |
| Kotlin | `app/src/main/java/com/foton/frontend/MainActivity.kt` | 388 lines, one file, one class |
| Gradle Kotlin DSL | `settings.gradle.kts`, `build.gradle.kts`, `app/build.gradle.kts` | 80 lines, no `dependencies {}` block anywhere |
| XML | `AndroidManifest.xml` + 8 resource files | 97 lines |
...
(`res/drawable/ic_menu_gear.xml`, 24dp vector, 8 teeth, `#7C4DFF`) with no
background plate - the 60% white scrim the platform glyph needed is gone. It
opens a `PopupMenu` carrying the three items - Refresh, Server URL...,
...
hidden while fullscreen, so immersive really is immersive.
...
| version | versionCode 6, versionName 0.1.1 | `app/build.gradle.kts` |
